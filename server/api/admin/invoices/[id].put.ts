...
export default defineEventHandler(async (event) => {
  ...
  if (!parsed.success) throw createError({ statusCode: 422, statusMessage: parsed.error.issues[0]?.message || 'Invalid invoice' })
  ...
  const invoice = await db.transaction(async (tx) => {
    ...
    if (!updated) throw createError({ statusCode: 409, statusMessage: 'Only draft invoices can be edited' })
