...
export default defineEventHandler(async (event) => {
  ...
  if (!parsed.success) throw createError({ statusCode: 422, statusMessage: parsed.error.issues[0]?.message || 'Ongeldige bedrijfsinstellingen' })
  ...
  if (!settings) throw createError({ statusCode: 500, statusMessage: 'Bedrijfsinstellingen opslaan is niet gelukt' })
  return { settings }
})
