...
export default defineEventHandler(async (event) => {
  ...
  for await (const value of event.node.req) {
    ...
    if (total > session.chunkSize) throw createError({ statusCode: 413, statusMessage: 'Chunk is te groot' })
    chunks.push(chunk)
  }

  await writeMediaUploadChunk(session, index, Buffer.concat(chunks, total))
