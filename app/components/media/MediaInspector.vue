...
const sourceLabel = computed(() => ({
  ...
  generated: 'Generated by NightLight',
...
async function save() {
  ...
  try {
    ...
    editing.value = false
    emit('changed')
    emit('notify', 'Metadata saved.')
  } catch (error) {
    emit('notify', apiErrorMessage(error, 'Could not save metadata.'), 'error')
  } finally {
    saving.value = false
  }
}
...
async function addTag() {
  ...
  try {
    ...
    emit('notify', apiErrorMessage(error, 'Could not add the tag.'), 'error')
  }
}
...
async function removeTag(tag: string) {
  try {
    ...
    emit('notify', apiErrorMessage(error, 'Could not remove the tag.'), 'error')
  }
}
...
async function copyLink() {
  try {
    ...
    emit('notify', 'Link copied to clipboard.')
  } catch {
...
<template>
  <Teleport to="body">
    <div class="layer">
      <button type="button" class="mh-backdrop" aria-label="Close preview" @click="emit('close')" />
      <section class="inspector" role="dialog" aria-modal="true" :aria-label="title">
        ...
        <div class="stage">
          <div class="preview" :data-kind="kind">
            ...
            <button v-if="position > 0" type="button" class="nav prev" aria-label="Previous asset" @click="go(-1)"><Icon name="lucide:chevron-left" aria-hidden="true" /></button>
            <button v-if="position >= 0 && position < sequence.length - 1" type="button" class="nav next" aria-label="Next asset" @click="go(1)"><Icon name="lucide:chevron-right" aria-hidden="true" /></button>
            <span v-if="position >= 0" class="counter">{{ position + 1 }} / {{ sequence.length }}</span>
          </div>

          <div v-if="familyItems.length > 1" class="strip">
            <h3>Original &amp; variants <span>{{ familyItems.length }}</span></h3>
            <div class="strip-items">
              ...
              >
                ...
                <span class="strip-label">{{ entry.parentAssetId ? entry.variantLabel || 'Variant' : 'Original' }}</span>
        ...
        <div class="details">
          <template v-if="!editing">
            ...
            <div class="tag-row">
              <button v-for="tag in item.tags" :key="tag" type="button" class="mh-tag removable" :title="`Remove ${tag}`" @click="removeTag(tag)">{{ tag }}</button>
              <form v-if="addingTag" class="tag-form" @submit.prevent="addTag">
                <input v-model="newTag" class="mh-input" list="inspector-tags" placeholder="New tag" autofocus @blur="addTag">
                <datalist id="inspector-tags"><option v-for="tag in tagSuggestions" :key="tag" :value="tag" /></datalist>
              </form>
              <button v-else type="button" class="mh-tag removable add-tag" @click="addingTag = true">+ Add tag</button>
            </div>

            <dl class="rows">
              <div>
                <dt><Icon name="lucide:calendar-days" aria-hidden="true" />Associated gig</dt>
                <dd>
              ...
              <div>
                <dt><Icon name="lucide:map-pin" aria-hidden="true" />Associated venue</dt>
                <dd>
              ...
              <div>
                <dt><Icon name="lucide:calendar-plus" aria-hidden="true" />Date added</dt>
                <dd>{{ formatMediaDate(item.createdAt, true) }}</dd>
              </div>
              <div>
                <dt><Icon name="lucide:inbox" aria-hidden="true" />Source</dt>
                <dd>{{ sourceLabel }}</dd>
              </div>
              <div v-if="parent">
                <dt><Icon name="lucide:git-branch" aria-hidden="true" />Variant of</dt>
              ...
              <div v-if="memberCollections.length">
                <dt><Icon name="lucide:folder" aria-hidden="true" />Collections</dt>
            ...
            <div class="usage">
              ...
              <p v-else class="muted">Not used anywhere yet.</p>
            </div>

            <div class="actions">
              <button type="button" class="mh-btn primary block" @click="editing = true"><Icon name="lucide:pencil" aria-hidden="true" />Edit metadata</button>
              <div class="action-pair">
                <button type="button" class="mh-btn" @click="copyLink"><Icon name="lucide:link" aria-hidden="true" />Copy link</button>
                <div class="split">
                  <button type="button" class="mh-btn" @click="download('original')"><Icon name="lucide:download" aria-hidden="true" />Download</button>
                  <MediaPopover v-model:open="downloadMenu" align="end" placement="top">
                    <template #trigger="{ toggle }">
                      <button type="button" class="mh-btn caret" aria-label="More download options" @click="toggle"><Icon name="lucide:chevron-down" aria-hidden="true" /></button>
                    </template>
                    <button type="button" class="mh-menu-item" @click="download('original')"><Icon name="lucide:file-down" aria-hidden="true" />Original file</button>
                    <button type="button" class="mh-menu-item" :disabled="familyItems.length < 2" @click="download('family')"><Icon name="lucide:folder-down" aria-hidden="true" />Original + {{ family.variants.length }} variant{{ family.variants.length === 1 ? '' : 's' }} (ZIP)</button>
                  </MediaPopover>
                </div>
              </div>
              <div class="action-pair">
                <button type="button" class="mh-btn ghost" @click="emit('uploadVariant', item)"><Icon name="lucide:layers" aria-hidden="true" />Upload variant</button>
                <button type="button" class="mh-btn ghost danger-text" @click="emit('delete', item)"><Icon name="lucide:trash-2" aria-hidden="true" />Delete</button>
              </div>
            </div>
          </template>
          ...
          <form v-else class="edit" @submit.prevent="save">
            ...
            <label class="mh-field"><span>Title</span><input v-model="form.title" class="mh-input" maxlength="240" :placeholder="title"></label>
            <label class="mh-field"><span>Alt text</span><textarea v-model="form.altText" class="mh-textarea" rows="2" maxlength="500" placeholder="Describe the media for accessibility" /></label>
            <label class="mh-field"><span>Tags</span><input v-model="form.tags" class="mh-input" placeholder="Comma separated"></label>
            <div class="two">
            ...
            <label v-if="form.parentAssetId" class="mh-field">
              ...
              <input v-model="form.variantLabel" class="mh-input" maxlength="80" placeholder="e.g. Story 9:16 crop">
            </label>
            <div class="action-pair">
              <button type="button" class="mh-btn" :disabled="saving" @click="editing = false; fillForm()">Cancel</button>
              <button type="submit" class="mh-btn primary" :disabled="saving">{{ saving ? 'Saving…' : 'Save' }}</button>
            </div>
          </form>
        </div>
      </section>
