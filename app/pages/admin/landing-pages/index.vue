...
async function createPage(){
  ...
  }catch(error:unknown){errorMessage.value=apiErrorMessage(error,'Could not create landing page.')}
  ...
  <div class="landing-admin">
    ...
    <form v-if="showCreate" class="create-card" @submit.prevent="createPage">
      ...
      <div class="grid">
        ...
        <label>Navigation label<input v-model="form.navLabel" required></label>
        <label class="wide">Title<input v-model="form.title" required></label>
        <label>Eyebrow<input v-model="form.eyebrow" required></label>
        <label>CTA label<input v-model="form.ctaLabel" required></label>
        <label class="wide">Intro<textarea v-model="form.intro" rows="3" required/></label>
        <label class="wide">Body<textarea v-model="form.body" rows="5" required/></label>
        <label class="wide">SEO title<input v-model="form.seoTitle" required></label>
        <label class="wide">SEO description<textarea v-model="form.seoDescription" rows="2" required/></label>
        <label>Order<input v-model.number="form.ordering" type="number" min="0"></label>
      </div>
      <div class="toggles">
        <label><input v-model="form.published" type="checkbox"> Published</label>
        <label><input v-model="form.showInNavigation" type="checkbox"> Show in navigation</label>
        <label><input v-model="form.indexable" type="checkbox"> Search indexable</label>
      </div>
      <p v-if="errorMessage" class="error">{{errorMessage}}</p>
      <button class="primary" type="submit" :disabled="saving">{{saving?'Creating…':'Create page'}}</button>
    </form>

    <div v-if="status==='pending'" class="empty">Loading landing pages…</div>
    <div v-else-if="!data?.pages.length" class="empty">No landing pages yet.</div>
    <div v-else class="page-list">
      <NuxtLink v-for="page in data.pages" :key="page.id" :to="`/admin/landing-pages/${page.id}`" class="row">
        ...
        <div class="flags">
          <span :class="{on:page.published}">{{page.published?'Published':'Draft'}}</span>
          <span :class="{on:page.showInNavigation}">{{page.showInNavigation?'In nav':'Hidden nav'}}</span>
