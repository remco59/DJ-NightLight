...
<AdminFilterBar :has-active-filters="Boolean(search)" :results-label="`${data?.venues.length??0} locaties`" @clear-all="search=''">
<template #primary><input v-model="search" type="search" placeholder="Zoek op locatie, plaats of contactpersoon…"></template>
<template #chips><AdminFilterChip v-if="search" :label="`Zoeken: ${search}`" @remove="search=''" /></template>
</AdminFilterBar>
<div v-if="status==='pending'" class="empty">Locaties laden…</div><div v-else-if="!data?.venues.length" class="empty">Geen locaties gevonden.</div>
