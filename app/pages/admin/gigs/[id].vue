...
function stripeLabel(invoice:InvoiceSummary){
  if(invoice.paymentStatus==='paid'||invoice.stripeStatus==='succeeded')return 'Paid via Stripe'
  if(invoice.stripeStatus==='pending'||invoice.paymentStatus==='pending')return 'Stripe payment pending'
  if(invoice.stripeStatus==='failed'||invoice.paymentStatus==='failed')return 'Stripe payment failed'
  if(invoice.stripeStatus==='expired')return 'Stripe checkout expired'
  return invoice.status==='finalized'?'Ready for Stripe payment':'Not sent to Stripe yet'
}
...
const titlePlaceholder=computed(()=>`Optional: leave empty to show “${gigDisplayTitle({venueName:venueName.value,eventType:form.eventType})}”`)
const publicTitlePlaceholder=computed(()=>`Optional: leave empty to show “${gigDisplayTitle({title:form.title,venueName:venueName.value,eventType:form.eventType})}”`)
...
<section v-if="canManageGigs" class="card invoice-card">
  <div class="section-title invoice-heading">
    <div><p class="eyebrow">Finance</p><h2>Invoices & Stripe</h2><span class="subtle-copy">Invoices belong to this gig. Stripe status updates automatically after checkout or bank transfer.</span></div>
    <button type="button" class="secondary" @click="createInvoice">Create invoice</button>
  </div>
  <div v-if="!data.invoices.length" class="subtle">No invoices for this gig yet.</div>
  <NuxtLink v-for="invoice in data.invoices" :key="invoice.id" :to="`/admin/invoices/${invoice.id}`" class="invoice-row">
    <div class="invoice-main">
      ...
      <span>{{invoice.status==='draft'?'Draft':`Issued ${invoiceDate(invoice.issueDate)} · due ${invoiceDate(invoice.dueDate)}`}}</span>
    </div>
    <div class="invoice-statuses">
      <span class="state" :data-state="invoice.paymentStatus">{{invoice.paymentStatus}}</span>
...
<section v-if="canManageGigs" class="card"><div class="section-title"><div><p class="eyebrow">Client portal</p><h2>Secure access</h2></div></div><div class="portal-actions"><label>Validity (days)<input v-model.number="portalDays" type="number" min="1" max="365"></label><button type="button" class="secondary" :disabled="portalBusy" @click="createPortalLink(false)">Create link</button><button type="button" class="primary" :disabled="portalBusy" @click="createPortalLink(true)">Resend invitation</button></div><div v-if="portalUrl" class="one-time-link"><div><strong>One-time result</strong><span>{{portalUrl}}</span></div><button type="button" class="with-icon secondary" @click="copyPortalUrl"><Icon name="lucide:copy" aria-hidden="true" />Copy</button></div><p v-if="portalMessage" class="portal-message">{{portalMessage}}</p><div v-if="!portalData?.links.length" class="subtle">No portal links have been issued.</div><div v-for="link in portalData?.links||[]" :key="link.id" class="portal-row"><div><strong>{{link.state}}</strong><span>Expires {{portalDate(link.expiresAt)}} · Last used {{portalDate(link.lastUsedAt)}}</span></div><button v-if="link.state==='active'" type="button" class="remove-small" @click="revokePortalLink(link.id)">Revoke</button></div></section>

<section v-if="canManageGigs&&submissionData" class="card"><div class="section-title"><div><p class="eyebrow">Portal submission</p><h2>Contract & music wishes</h2></div><NuxtLink to="/admin/questionnaire" class="text-button">Edit template</NuxtLink></div><div class="submission-status"><strong>{{submissionData.status.replace('_',' ')}}</strong><span>Questionnaire version {{submissionData.templateVersion}}<template v-if="submissionData.submission?.submittedAt"> · {{portalDate(submissionData.submission.submittedAt)}}</template></span></div><div v-if="submissionData.submission" class="answer-grid"><div v-for="field in submissionData.fields" :key="field.id"><span>{{field.label}}</span><strong>{{answerValue(submissionData.submission.answers[field.id])}}</strong></div><div><span>Accepted by</span><strong>{{submissionData.submission.acceptedName||'—'}}</strong></div></div><div v-else class="subtle">The client has not submitted the questionnaire yet.</div><h3>Music wishes</h3><div v-if="!submissionData.wishes.length" class="subtle">No music wishes submitted.</div><div v-for="wish in submissionData.wishes" :key="wish.id" class="wish-row"><span>{{wish.category.replaceAll('_',' ')}}</span><div><strong>{{[wish.artist,wish.title].filter(Boolean).join(' — ')||wish.note||'Untitled wish'}}</strong><a v-if="wish.spotifyUrl" :href="wish.spotifyUrl" target="_blank" rel="noreferrer">Open Spotify</a><small v-if="wish.note">{{wish.note}}</small></div></div></section>
