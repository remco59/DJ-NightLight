...
const {user}=useUserSession();const canManageGigs=computed(()=>user.value?.role==='owner'||user.value?.role==='manager');const isOwner=computed(()=>user.value?.role==='owner')
...
type Detail={gig:Gig;contacts:Contact[];timeline:Timeline[];activity:Activity[];removalMode:'delete'|'archive'|null;options:{clients:ClientOption[];venues:VenueOption[];djs:DjOption[]}}
...
async function remove(){
  const mode=data.value?.removalMode
  if(!mode)return
  const prompt=mode==='archive'
    ? 'This gig has financial history. It will be archived and hidden, while invoices and payments are retained. Continue?'
    : 'This declined gig has no financial history and will be permanently deleted. This cannot be undone. Continue?'
...
<header class="hero"><div><p class="eyebrow">Gig</p><h1>{{data.gig.title}}</h1><div class="summary"><span>{{data.gig.status}}</span><span>{{data.gig.clientCompanyName||[data.gig.clientFirstName,data.gig.clientLastName].filter(Boolean).join(' ')||'No client'}}</span><span>{{data.gig.venueName||'No venue'}}</span></div></div><div v-if="canManageGigs" class="hero-actions"><button class="secondary" type="button" @click="createInvoice">Create invoice</button><button class="secondary" type="button" @click="duplicate">Duplicate</button><div v-if="isOwner&&data.removalMode" class="remove-action"><button class="danger" type="button" @click="remove">{{data.removalMode==='archive'?'Archive':'Delete permanently'}}</button><small>{{data.removalMode==='archive'?'Financial history is retained; the gig is hidden everywhere.':'No financial history exists; deletion is permanent.'}}</small></div></div></header>
