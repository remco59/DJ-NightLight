...
<header class="page-header"><div><p class="eyebrow">Operations</p><h1>Gigs</h1><p>Leads, bookings and your complete planning.</p></div><button v-if="canManageGigs" class="primary" @click="showCreate=!showCreate">{{showCreate?'Close':'New gig'}}</button></header>
<div v-if="removalNotice" class="notice">{{removalNotice}}</div>

<form v-if="showCreate&&canManageGigs" class="create-card" @submit.prevent="createGig"><h2>New gig</h2><div class="form-grid">
