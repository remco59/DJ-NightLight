...
const leadVisual = computed(() => {
  if (content.value?.publicCopy.visuals.aboutLeadImageUrl) {
    return { url: content.value.publicCopy.visuals.aboutLeadImageUrl, alt: content.value.publicCopy.visuals.aboutLeadAlt }
  }
  if (content.value?.heroImageUrl) {
    return { url: content.value.heroImageUrl, alt: content.value.publicCopy.visuals.aboutLeadAlt }
  }
...
<template>
  <main v-if="content" class="public-page about-page">
    <div class="public-container">
      ...
      <section class="story">
        <p class="story-kicker">{{ content.publicCopy.about.storyTitle }}</p>
        <div class="story-copy">
          <p>{{ content.publicCopy.about.storyBody1 }}</p>
          <p>{{ content.publicCopy.about.storyBody2 }}</p>
        </div>
      </section>
      ...
      <figure class="room-visual">
        ...
        <figcaption>
          ...
          <blockquote>“{{ content.publicCopy.about.momentQuote }}”</blockquote>
          <span>{{ content.publicCopy.about.momentBody }}</span>
        </figcaption>
      </figure>

      <section class="principles-wrap">
        <div class="principles-intro">
          <p class="eyebrow">{{ content.publicCopy.about.principlesEyebrow }}</p>
          <h2>{{ content.publicCopy.about.principlesTitle }}</h2>
        </div>
...
.about-cta{display:grid;grid-template-columns:.55fr 1fr;gap:3rem;align-items:start;padding:clamp(6rem,10vw,10rem) 0;border-top:1px solid #27232c}.about-cta h2{max-width:15ch;white-space:pre-line;margin:0 0 2rem;font-size:clamp(3rem,6vw,6rem);line-height:.93;letter-spacing:-.065em}
