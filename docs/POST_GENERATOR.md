...
Before Remotion sees demanding video sources, the worker probes them with FFprobe. HEVC/AV1/VPx and other non-H.264 codecs, >8-bit video, HDR/BT.2020 sources and media above 1920 px on either axis are transcoded once to a cached H.264/yuv420p render proxy under `generated/render-proxies/`. HDR sources are tone-mapped to BT.709 SDR. The original upload is never changed, and later exports reuse the proxy as long as it is newer than the source. This keeps Remotion `OffthreadVideo` frame extraction predictable for modern phone footage while retaining the original in the media library. A 60-second delay-render timeout remains as a safety margin.
